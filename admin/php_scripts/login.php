     ...
     if(empty($email)){
        ...
        if($user_row != null){
            if($user_row['password'] == md5($password) && $user_row['role'] == "admin"){
                $_SESSION["auth_success"] = ["id" => $user_row['id'],"name" => $user_row['user_name'],"email" => $user_row['email']];
                header("location: dashboard.php ");
            }if($user_row['password'] == md5($password) && $user_row['role'] == "member"){
                $_SESSION["member_success"] = ["id" => $user_row['id'],"name" => $user_row['user_name'],"email" => $user_row['email']];
